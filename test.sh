...
runner "$PHP_PATH vendor/bin/phpcbf --standard=./csruleset.xml Kairos/"
...
test "$PHP_PATH vendor/bin/phpcs --standard=./csruleset.xml Kairos/" PHPCS 100
echo "$TEST_RES" > doc/phpcs.txt

test "$COMPOSER_PATH validate" COMPOSER 100
echo "$TEST_RES" > doc/composer.txt

test "$PHP_PATH vendor/bin/phpmd Kairos/ text ./phpmd.xml" PHPMD 100
echo "$TEST_RES" > doc/phpmd.txt

test "$PHP_PATH vendor/bin/phpcpd Kairos/" PHPCPD 1
